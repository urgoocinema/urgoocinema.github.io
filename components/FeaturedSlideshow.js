...
export class FeaturedSlideshow extends HTMLElement {
  ...
  renderSlides() {
    ...
    const slidesHTML = this.featuredMovies.map((movie, index) => `
      <article class="mySlides fade" data-movie-id="${movie.id}">
        ...
        <img 
          src="${this.getLandscapePosterUrl(movie.poster_url)}" 
          alt="${movie.title}"
  ...
  getLandscapePosterUrl(portraitUrl) {
    // Convert portrait poster URLs to landscape versions
    // e.g., "snowwhite-portrait.webp" -> "snowwhite.webp"
    if (!portraitUrl) return portraitUrl;
    
    return portraitUrl.replace('-portrait', '');
  }
  ...
  setupSlideshow() {
    setTimeout(() => {
      ...
      console.log('Setting up slideshow, container found:', !!container);
      ...
      // Add event listeners for booking buttons using event delegation
      if (container) {
  ...
  handleBookingClick(movieId) {
    console.log('handleBookingClick called with movieId:', movieId, 'type:', typeof movieId);
    console.log('Available movies:', this.featuredMovies.map(m => ({ id: m.id, type: typeof m.id, title: m.title })));
    
    // Find the movie data - convert movieId to number for comparison
    const movie = this.featuredMovies.find(m => m.id == movieId || m.id === parseInt(movieId));
    if (!movie) {
      console.error('Movie not found with ID:', movieId);
      return;
    }

    console.log('Dispatching movie-booking-requested event for:', movie);
